    ...
    <nav className="flex flex-wrap items-center justify-between p-3 bg-gradient-to">
      ...
      >
        ...
        <ul className="flex flex-col md:flex-row gap-4 md:gap-25 text-base font-bold mt-4 md:mt-0">
          ...
          {/* Auth Buttons (Right) */}
          <div className="flex flex-col md:flex-row gap-2 font-semibold mt-4 md:mt-0 text-sm">
            {navData.auth.map((item, index) => {
              ...
              return (
                <Link
                  key={index}
                  href={item.url}
                  className={`group inline-flex items-center gap-1 px-3 py-1.5 rounded-md border transition-all duration-300 ease-in-out transform hover:-translate-y-0.5 hover:shadow-md ${
                    isGetStarted
                ...
                >
                  <span className="w-4 h-4 transition-transform duration-300 group-hover:rotate-6">
                    {isGetStarted ? (
